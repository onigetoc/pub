...
exports.LinkedInExtractor = void 0;
const _base_1 = require("./_base");
const dom_1 = require("../utils/dom");
const comments_1 = require("../utils/comments");
class LinkedInExtractor extends _base_1.BaseExtractor {
    ...
    extract() {
        ...
        const contentHtml = (0, comments_1.buildContentHtml)('linkedin', postContent, comments);
    ...
    getPostContent() {
        ...
        const textEl = this.postArticle.querySelector('.update-components-text.update-components-update-v2__commentary');
    ...
    cleanTextContent(el) {
        ...
        let html = (0, dom_1.serializeHTML)(clone).trim();
        // Strip HTML comments
        html = html.replace(/<!--.*?-->/g, '');
    ...
    extractImages() {
        if (!this.postArticle)
            return '';
        const images = [];
        this.postArticle.querySelectorAll('.update-components-image img, .feed-shared-image img').forEach(img => {
    ...
    extractVideo() {
        ...
        return `<img src="${(0, dom_1.escapeHtml)(poster)}" alt="Video thumbnail" />`;
    }
    extractComments() {
        if (!this.postArticle)
            return '';
        const commentData = [];
        const topLevelComments = this.postArticle.querySelectorAll('article.comments-comment-entity:not(.comments-comment-entity--reply)');
        for (const comment of Array.from(topLevelComments)) {
            const data = this.extractCommentData(comment, 0);
            if (data)
                commentData.push(data);
            const replies = comment.querySelectorAll('.comments-replies-list article.comments-comment-entity--reply');
            for (const reply of Array.from(replies)) {
                const replyData = this.extractCommentData(reply, 1);
                if (replyData)
                    commentData.push(replyData);
            }
        }
        return commentData.length > 0 ? (0, comments_1.buildCommentTree)(commentData) : '';
    }
    extractCommentData(comment, depth) {
    ...
    createDescription() {
        ...
        const quotedWrapper = this.postArticle.querySelector('.feed-shared-update-v2__update-content-wrapper');
        const textEl = this.postArticle.querySelector('.update-components-text.update-components-update-v2__commentary');
        if (!textEl || (quotedWrapper && quotedWrapper.contains(textEl)))
            return '';
...
exports.LinkedInExtractor = LinkedInExtractor;

...
class MediumExtractor extends _base_1.BaseExtractor {
    ...
    canExtract() {
        ...
        if (this.article.classList?.contains('meteredContent'))
            return true;
    ...
    cleanArticle() {
        ...
        this.article.querySelectorAll('p, span, div').forEach(el => {
            ...
            if (/^\w{3}\s+\d{1,2},\s+\d{4}/.test(text) && text.length < 30) {
                el.remove();
                return;
            }
            if (/^·\s*\d+\s*\w+\s*ago$/.test(text)) {
                el.remove();
                return;
            }
            if (/^·?\s*\d+\s*min\s*read$/.test(text))
                el.remove();
        });
    }
    getTitle() {
        ...
        if (storyTitle)
            return storyTitle.textContent?.trim() || '';
    ...
    getDescription() {
        ...
        for (const p of Array.from(paragraphs)) {
            ...
            if (text.length < 3 || /^[\d\W]+$/.test(text))
                continue;

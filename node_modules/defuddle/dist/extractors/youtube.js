...
class YoutubeExtractor extends _base_1.BaseExtractor {
    ...
    formatDescription(description) {
        return `<p>${(0, dom_1.escapeHtml)(description).replace(/\n/g, '<br>')}</p>`;
    }
    getVideoData() {
        const videoId = this.getVideoId();
        ...
        for (const script of scripts) {
            try {
                ...
                if (videoObject && videoObject.description)
                    return videoObject;
                ...
                if (videoObject && (videoObject.comment || !fallbackVideoObject))
                    fallbackVideoObject = videoObject;
            }
            catch {
                // ignore invalid JSON
            }

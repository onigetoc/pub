...
exports.LOOKS_LIKE_LATEX_RE = exports.mathSelectors = exports.mathFastCheck = exports.isBlockDisplay = exports.getBasicLatexFromElement = exports.getMathMLFromElement = exports.reconstructMathMLFromMjx = void 0;
...
const normalizeMathGlyphs = (s) => s.normalize('NFKC').replace(INVISIBLE_MATH_CHARS, '');
...
const flattenMjxChildren = (node, doc, skip) => {
    if (!node)
        return [];
    const out = [];
    for (const child of Array.from(node.children)) {
...
const convertMjxNode = (node, doc) => {
    ...
    switch (name) {
        ...
        case 'munderover': {
            ...
            if (name === 'munder')
                return [buildEl('munder', [base, wrapParts(flattenMjxChildren(under, doc), doc)], doc)];
            if (name === 'mover')
                return [buildEl('mover', [base, wrapParts(flattenMjxChildren(over, doc), doc)], doc)];
 ...
 */
const reconstructMathMLFromMjx = (mjxMath, doc) => {
    const math = doc.createElement('math');
    math.setAttribute('xmlns', 'http://www.w3.org/1998/Math/MathML');
    for (const child of Array.from(mjxMath.children)) {
        for (const n of convertMjxNode(child, doc))
            math.appendChild(n);
    }
    if (math.childNodes.length === 0)
        return null;
...
exports.reconstructMathMLFromMjx = reconstructMathMLFromMjx;
/**
 ...
 */
const flattenTaggedEquationTables = (mathEl) => {
    ...
    for (const table of tables) {
        ...
        if (rows.length !== 1)
            continue;
        const row = rows[0];
        if (row.tagName.toLowerCase() !== 'mlabeledtr')
            continue;
        ...
        if (labelCell.childNodes.length > 0) {
            ...
            while (labelCell.firstChild)
                replacement.appendChild(labelCell.firstChild);
        }
        table.replaceWith(replacement);
    }
};
/** Clone a math element, flatten tagged-equation tables, and serialize it. */
const serializeNormalizedMathML = (mathEl) => {
...
const getMathMLFromElement = (el) => {
    ...
    if (mjxMath) {
        const doc = el.ownerDocument || document;
        const reconstructed = (0, exports.reconstructMathMLFromMjx)(mjxMath, doc);
        if (reconstructed)

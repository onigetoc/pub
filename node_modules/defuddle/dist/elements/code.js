...
exports.codeBlockRules = [
    {
        ...
        transform: (el, doc) => {
            ...
            el.querySelectorAll('[class*="header"], [class*="toolbar"], [class*="titlebar"], [class*="title-bar"]').forEach(elem => {
                const tag = elem.tagName;
                if (tag !== 'DIV' && tag !== 'SPAN')
                    return;
                const lineAncestor = elem.closest?.('[data-line], .line');
                if (lineAncestor && el.contains(lineAncestor))
                    return;
                if (elem.querySelector('[data-line], .line, pre'))
                    return;
                const text = (elem.textContent || '').trim();
                if ((0, utils_1.countWords)(text) <= 5) {
                    elem.remove();
                }
            });
            ...
            if (cmContent && !language) {
                ...
                for (const div of allDivs) {
                    if (div.contains(cmContent))
                        continue; // skip code area and its ancestors
            ...
            if (!codeContent && cmContent) {
                ...
                if (el.tagName !== 'PRE' && el.tagName !== 'CODE') {
                    ...
                    const codePre = pres.find(p => p.querySelector('code[data-lang], code[class*="language-"], .line, [data-line]')) || pres.find(p => p.querySelector('span[class]') && !p.classList.contains('lineno'));
                    if (codePre) {
            ...
            for (let i = 0; i < 3 && ancestor; i++) {
                ...
                if (container.closest?.('[data-callout]'))
                    break;

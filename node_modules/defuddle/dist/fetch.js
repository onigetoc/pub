...
exports.BOT_UA_DOMAINS = exports.BOT_UA = exports.DEFAULT_UA = exports.FETCH_TIMEOUT = void 0;
...
exports.FETCH_TIMEOUT = 10000; // 10s
...
function getProxyUrl(targetUrl) {
    ...
    const raw = (isHttps ? (process.env.HTTPS_PROXY || process.env.https_proxy) : undefined) ||
...
function validateAndDecode(contentType, contentLength, buffer) {
...
function proxyGet(targetUrl, proxy, reqHeaders) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            ...
            done(new Error(`Timed out fetching page after ${exports.FETCH_TIMEOUT / 1000}s`));
        }, exports.FETCH_TIMEOUT);
        ...
        const collectResponse = (res) => {
            ...
            res.on('data', (chunk) => {
                ...
                if (size > MAX_SIZE) {
                    res.destroy();
                    done(new Error(`Page too large (>${MAX_SIZE / 1024 / 1024}MB)`));
                }
                else
                    chunks.push(chunk);
            });
        ...
        const outHeaders = { ...reqHeaders, 'Host': target.hostname };
        if (isHttps) {
            const connectHeaders = { 'Host': `${target.hostname}:${targetPort}` };
            if (proxyAuth)
            ...
            if (proxyAuth)
                outHeaders['Proxy-Authorization'] = proxyAuth;
            const req = http.request({
                host: proxy.hostname,
                port: proxyPort,
                method: 'GET',
...
async function fetchPageViaProxy(targetUrl, proxy, reqHeaders, redirectsLeft = 10) {
...
async function fetchPage(targetUrl, userAgent, language) {
    const headers = {
    ...
    const timer = setTimeout(() => controller.abort(), exports.FETCH_TIMEOUT);
    try {
        ...
        if (err.name === 'AbortError') {
            throw new Error(`Timed out fetching page after ${exports.FETCH_TIMEOUT / 1000}s`);
        }
        throw err;
    }
    finally {

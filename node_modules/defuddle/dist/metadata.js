...
class MetadataExtractor {
    ...
    static firstValid(thunks) {
        for (const thunk of thunks) {
            const v = thunk();
            if (v && !this.isPlaceholderValue(v))
                return v;
        }
        return '';
    }
    static getAuthor(doc, schemaOrgData, metaTags) {
        ...
        let authorsStrings = this.getMetaContents(metaTags, "name", "citation_author").filter(s => !this.isPlaceholderValue(s));
        ...
        if (relAuthorEls.length > 0 && relAuthorEls.length <= 3) {
            ...
            if (uniqueRelNames.length > 0)
                return uniqueRelNames.join(', ');
        }
        ...
        if (collectedAuthorsFromDOM.length > 0) {
            ...
            if (uniqueAuthors.length > 1) {
                uniqueAuthors = uniqueAuthors.filter(a => !uniqueAuthors.some(b => b !== a && a.includes(b)));
            }
    ...
    static getSiteName(schemaOrgData, metaTags) {
        ...
        if (candidate && (0, utils_1.countWords)(candidate) > 6) {
            return '';
        }
        return candidate;
    }
    ...
    static isSiteIdentifier(candidate, authorNorm, siteNorm, domainNorm) {
        ...
        if (authorNorm && norm === authorNorm)
            return true;
        if (siteNorm && norm === siteNorm)
            return true;
        if (domainNorm) {
            const candidateNorm = norm.replace(/[^a-z0-9]/g, '');
            if (candidateNorm === domainNorm)
                return true;
        }
        return false;
    }
    static cleanTitle(title, siteName) {
        ...
        if (siteName && siteName.toLowerCase() !== title.toLowerCase() && (0, utils_1.countWords)(siteName) <= 6) {
            ...
            if (allPositions.length > 0) {
                ...
                if (lastSegment && siteNameLower.includes(lastSegment)) {
                    ...
                    for (let i = allPositions.length - 2; i >= 0; i--) {
                        ...
                        if ((0, utils_1.countWords)(segment) > 3)
                            break;
                        cutIndex = pos.index;
                    }
    ...
    static trySeparatorSplit(title, pattern, opts) {
        let match;
        const positions = [];
        ...
        if (opts.guard((0, utils_1.countWords)(suffixTitle), (0, utils_1.countWords)(suffixSite))) {
        ...
        if (!opts.suffixOnly) {
            ...
            if (opts.guard((0, utils_1.countWords)(prefixTitle), (0, utils_1.countWords)(prefixSite))) {
    ...
    static getPublished(doc, schemaOrgData, metaTags, url) {
        ...
        const h1 = doc.querySelector('h1');
        if (h1) {
            const scan = (start, step, childrenOnly) => {
                let sibling = start;
                for (let i = 0; i < 3 && sibling; i++) {
    ...
    static getTimeElement(doc, url) {
        ...
        for (const element of Array.from(doc.querySelectorAll('time'))) {
            if (this.isLinkedToOtherPage(element, url))
                continue;
    ...
    static isLinkedToOtherPage(el, pageUrl) {
        if (!pageUrl)
            return false;
        ...
        try {
            ...
            if (target.origin !== current.origin)
                return false; // external link — keep

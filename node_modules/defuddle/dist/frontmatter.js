 ...
 */
function buildFrontmatter(result, sourceUrl) {
    const lines = ['---'];
    // Escape a string for use as a YAML double-quoted value
    const esc = (s) => s.replace(/"/g, '\\"').replace(/\n/g, ' ');
    if (result.title)
        lines.push(`title: "${esc(result.title)}"`);
    if (result.author)
        lines.push(`author: "${esc(result.author)}"`);
    if (result.site)
        lines.push(`site: "${esc(result.site)}"`);
    if (result.published)
    ...
    if (result.description) {
        const desc = (0, utils_1.countWords)(result.description) > 300
            ? truncateWords(result.description, 300)
            : result.description;
        lines.push(`description: "${esc(desc)}"`);
    }
    if (result.wordCount)
        lines.push(`word_count: ${result.wordCount}`);
    lines.push('---');
    return lines.join('\n') + '\n\n';
}

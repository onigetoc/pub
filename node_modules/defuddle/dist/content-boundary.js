...
exports.findContentStart = findContentStart;
exports.isAboveContentStart = isAboveContentStart;
...
function isProseBlock(el) {
    if (!el.tagName)
        return false;
    if (!CANDIDATE_TAGS.has(el.tagName))
        return false;
    if (el.closest(SKIP_ANCESTOR_SELECTOR))
        return false;
    const className = typeof el.className === 'string' ? el.className : '';
    if (HIDDEN_CLASS_RE.test(className))
        return false;
    ...
    if (el.querySelector(DIALOG_SELECTOR))
        return false;
    ...
    // tends to be longer.
    if (BYLINE_PATTERN.test(text) && words < 15)
        return false;
    if (DATE_PATTERN.test(text) && words < 20)
        return false;
    if (linkTextLength(el) > text.length * 0.7)
        return false;
    ...
    if (el.tagName === 'DIV' && !el.querySelector('p'))
        return false;
    return true;
}
/**
 ...
 */
function findContentStart(mainContent, title) {
    ...
    if (start)
        return findContentStart(mainContent, '');
    return null;
}
/**
 ...
 */
function isAboveContentStart(el, boundary) {
    if (!boundary)
        return false;
    if (el === boundary)
        return false;
...
//# sourceMappingURL=content-boundary.js.map
...
function removeBySelector(doc, debug, removeExact = true, removePartial = true, mainContent, debugRemovals, skipHiddenExactSelectors = false) {
    ...
    if (removePartial) {
        ...
        const individualRegexes = debug
            ? constants_1.PARTIAL_SELECTORS.map(p => ({ pattern: p, regex: new RegExp(p, 'i'), anchored: new RegExp('^(?:' + p + ')$', 'i') }))
            : null;
        ...
        allElements.forEach(el => {
            ...
            const attrsMatch = hasAttrs && constants_1.PARTIAL_SELECTORS_REGEX.test(attrs);
            const idHasDelimiter = id ? /[\s_\-:.]/.test(id) : false;
            const idMatch = id
                ? (idHasDelimiter ? constants_1.PARTIAL_SELECTORS_REGEX.test(id) : constants_1.PARTIAL_SELECTORS_ANCHORED_REGEX.test(id))
                : false;
